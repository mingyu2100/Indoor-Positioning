...
x_DB = np.loadtxt('Geomagnetic_1m_ir.csv', delimiter=',', dtype=np.float32)
x_data = np.loadtxt('Geomagnetic_1m_mg.csv', delimiter=',', dtype=np.float32)
...
Y = tf.placeholder(tf.int32, [None, 1], name='label') # [None, db_classes]
Y_one_hot = tf.one_hot(Y, db_classes - read_num + 1)
Y_one_hot = tf.reshape(Y_one_hot, [-1, db_classes - read_num + 1])

with tf.name_scope('layer1'):
    W1 = tf.Variable(tf.random_normal([40, 40], name='weight1'))
    # W1 = tf.get_variable('W1', shape=[40, 128], initializer=tf.contrib.layers.xavier_initializer())
...
with tf.name_scope('layer2'):
    # W2 = tf.Variable(tf.random_normal([20, db_classes - read_num + 1], name='weight2'))
    W2 = tf.get_variable('W2', shape=[40, db_classes - read_num + 1], initializer=tf.contrib.layers.xavier_initializer())
...
# 보간법 사용 후 그래프
plt.figure()
plt.plot(Interpol_length_db, Interpor_m, 'o', linewidth=1, label='Interpol_m_db')
plt.plot(Interpol_length_db, Interpor_x, 'o', linewidth=1, label='Interpol_x_db')
plt.plot(Interpol_length_db, Interpor_y, 'o', linewidth=1, label='Interpol_y_db')
plt.plot(Interpol_length_db, Interpor_z, 'o', linewidth=1, label='Interpol_z_db')
